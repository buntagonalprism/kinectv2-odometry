...
#ifndef MY_DRAWING_UTILS_H
#define MY_DRAWING_UTILS_H
...
namespace MyDrawingUtils {
	...
	void DrawKeypoints(Mat& gimage_in, vector<KeyPoint>& kps_in);

	void DrawMatches(vector<Mat>& gimages, vector<frame_kps>& kps, vector<DMatch>& good_matches, int current_frame);

	void DrawKeypointTracks(Mat& gimage_current, std::vector<world_pt>& world_pts, int numframes, int startframe = 0, int increment = 1);

	void EigenToPcl(Eigen::Matrix<float, 3, Eigen::Dynamic>& eigen_in, pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_out);

	void DisplayFeatureClouds(vector<Eigen::Matrix<float, 3, Eigen::Dynamic>>& kp_clouds, Eigen::Matrix<float, 4, 4> relative_pose, int current_frame);

	void DisplayDenseClouds(vector<Mat>& dimages, int current_frame);
